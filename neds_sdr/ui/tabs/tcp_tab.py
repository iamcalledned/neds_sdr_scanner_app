...
class TcpTab(QtWidgets.QWidget):
    ...
    def refresh_table(self):
        ...
        for row, (name, receiver) in enumerate(dongles.items()):
            ...
            self.table.setItem(row, 1, QtWidgets.QTableWidgetItem(receiver.host))
            self.table.setItem(row, 2, QtWidgets.QTableWidgetItem(str(receiver.port)))

            # Gain slider
            gain_slider = QtWidgets.QSlider(QtCore.Qt.Orientation.Horizontal)
            gain_slider.setMinimum(0)
            gain_slider.setMaximum(50)
            gain_slider.setValue(int(receiver.gain))
            ...
            # Status
            status_text = "Connected" if receiver.running else "Offline"
            status_item = QtWidgets.QTableWidgetItem(status_text)
            status_item.setForeground(QtGui.QColor("green" if receiver.running else "red"))
